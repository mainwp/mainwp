...
mainwp_fix_all_security_issues = function ( siteId, refresh ) {
  ...
  jQuery.post( ajaxurl, data, function ( pRefresh, pElement ) {
...
jQuery( document ).ready( function () {
    jQuery( '#mainwp-sync-sites' ).on( 'click', function ( event ) {
      mainwp_sync_sites_data();
    } );

    // to compatible with extensions
    jQuery('#dashboard_refresh').on('click', function (event) {
        mainwp_sync_sites_data();
    });
});
...
jQuery( document ).ready( function () {
  ...
  jQuery( document ).on( 'click', '#mainwp-bulk-trust-plugins-action-apply', function ( event ) {
    ...
    jQuery( document ).on( 'click', '#mainwp-bulk-trust-themes-action-apply', function ( event ) {
...
updatesoverview_themes_unignore_globally_all = function ( slug ) {
    var data = mainwp_secure_data( {
        action: 'mainwp_unignorepluginsthemes',
        type: 'theme',
        slug: '_ALL_'
...
updatesoverview_group_upgrade_translation_all = function ( id, groupId, obj ) {
    var msg = __( 'Are you sure you want to update all translations?' );
    mainwp_confirm(msg, function(){
...
updatesoverview_group_upgrade_plugin_all = function ( id, groupId, obj ) {
    var msg = __( 'Are you sure you want to update all plugins?' );
    mainwp_confirm(msg, function(){
...
updatesoverview_group_upgrade_theme_all = function ( id, groupId, obj ) {
    var msg = __( 'Are you sure you want to update all themes?' );
    mainwp_confirm(msg, function(){
...
updatesoverview_upgrade_plugintheme_all = function ( what, id, noCheck ) {
    ...
    updatesoverviewContinueAfterBackup = function ( pId, pWhat ) {
        return function ()
        {
            ...
            jQuery( "#wp_" + pWhat + "_upgrades_" + pId + " tr[updated=0]" ).each( function ( index, value ) {
...
updatesoverview_group_upgrade_plugintheme_all = function ( what, id, noCheck, groupId ) {
    ...
    updatesoverviewContinueAfterBackup = function ( pId, pWhat ) {
        return function ()
        {
            var list = [ ];
            var slug_att = pWhat + '_slug';
            jQuery( "#wp_" + pWhat + "_upgrades_" + pId + '_group_' + groupId + " tr[updated=0]" ).each( function ( index, value ) {
                var slug = jQuery(this).attr( slug_att );
                if ( slug ) {
                    list.push( slug );
                }
...
//                                parent.find( 'td:last-child' ).html( '<span style="color: #a00;"><i class="exclamation circle icon"></i> ' + __( 'Update failed!' ) + '</span>' );
...
jQuery( document ).ready( function () {
    ...
    jQuery('#mainwp-manage-sites-table').on( 'click', '.mainwp_site_reconnect', function ( event ) {
      mainwp_managesites_reconnect( jQuery( this ) );
      return false;
    } );

    jQuery( '.mainwp-updates-overview-reconnect-site' ).on( 'click', function ( event ) {
        ...
        return function ( response ) {
            ...
            if ( response.substr( 0, 5 ) == 'ERROR' ) {
                ...
                if ( response.length == 5 ) {
                  error = 'Undefined error! Please try again. If the process keeps failig, please contact the MainWP support.';
                } else {
...
mainwp_ss_select = function ( me, val ) {
    ...
    } else if ( tab == 'stagging' ) {
      parent.find( '#mainwp-select-stagging-sites-list .item:not(.no-select) INPUT:enabled:checkbox' ).attr( 'checked', val ).change();
    } else { //group
...
mainwp_sites_selection_onvisible_callback = function ( me ) {
    ...
    if (selected_tab == 'stagging') {
        ...
        // uncheck stagging sites
        parent.find( '#mainwp-select-stagging-sites-list INPUT:checkbox' ).attr( 'checked', false );
        parent.find( '#mainwp-select-groups-list INPUT:checkbox' ).attr( 'checked', false );
...
mainwp_newpost_updateCategories = function ()
{
    ...
    if ( catsSelection.length > 0 )
    {
        ...
        if ( tab == 'site' ) {
            ...
            sites = jQuery.map( jQuery( '#mainwp-select-stagging-sites-list INPUT:checkbox:checked' ), function ( el, i ) {
                return jQuery( el ).val();
            } );
        } else { //group
            groups = jQuery.map( jQuery( '#mainwp-select-stagging-sites-list INPUT:checkbox:checked' ), function ( el, i ) {
                return jQuery( el ).val();
            } );
        }
...
jQuery( document ).ready( function () {
    ...
    jQuery( '#bulk_import_createuser' ).on( 'click', function ( event ) {
...
// not used?
    ...
    jQuery( document ).on( 'click', '#mainwp_plugin_bulk_install_btn', function (event) {
    ...
    jQuery( document ).on( 'click', '#mainwp_theme_bulk_install_btn', function (event) {
        var act = jQuery(this).attr('bulk-action');
        if (act == 'install') {
...
backup_download_file = function ( pSiteId, type, url, file, regexfile, size, subfolder, remote_destinations )
{
    ...
    jQuery.ajax( {
        ...
        data: data,
        method: 'POST',
        success: function ( pSiteId, pFile, pRegexFile, pSubfolder, pRemoteDestinations, pSize, pType, pUrl, pData ) {
            return function ( response ) {
                backupDownloadRunning = false;
        ...
        error: function ( pSiteId, pFile, pRegexFile, pSubfolder, pRemoteDestinations, pSize, pType, pUrl, pData ) {
            return function () {
                //Try again in 10seconds
                /*setTimeout(function() {
...
serverinfo_prepare_download_info = function () {
    var report = "";
...
jQuery( document ).on( 'click', '#mainwp-download-system-report', function () {
    serverinfo_prepare_download_info();
...
mainwp_managesites_bulk_remove_next = function () {
  ...
  if ( ( bulkManageSitesTotal > 0 ) && ( bulkManageSitesFinished == bulkManageSitesTotal ) ) {
    ...
    setHtml( '#mainwp-message-zone', __( "Process complated. Reloading page..." ) );
    setTimeout( function () {
      window.location.reload()
    }, 3000 );
  }
...
mainwp_managesites_bulk_refresh_favico = function() {
    ...
    if ((bulkManageSitesTotal > 0) && (bulkManageSitesFinished == bulkManageSitesTotal)) {
        setHtml('#mainwp-message-zone', __("Refresh favico finished."));
    }
}
...
mainwp_managesites_refresh_favico_specific = function(pCheckedBox) {
    ...
    jQuery.ajax({
        ...
        success: function(pLoadingEl, pSiteid) { return function (response) {
            ...
            if (response.error != undefined)
            {
                result = '<span class="mainwp-red"><i class="fa fa-exclamation" aria-hidden="true"></i> ' + __('Download site icon failed') + ': ' + getErrorMessage(response.error) + '</div>';
...
jQuery( document ).ready( function () {
    if ( jQuery('body.mainwp-ui').length > 0 ) {
        jQuery('.mainwp-ui-page .ui.dropdown').dropdown();
