...
class MainWPCache
{
    ...
    public static function getCachedContext($page)
    {
        ...
        if ($cachedSearch != null && isset($cachedSearch['status'])) $cachedSearch['status'] = explode(',', $cachedSearch['status']);
    ...
    {
        if (session_id() == '') session_start();
        if (isset($_SESSION['MainWP' . $page . 'Search'])) echo $_SESSION['MainWP' . $page . 'Search'];
    }
}