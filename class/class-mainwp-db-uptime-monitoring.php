...
`up_statuscodes_json` text NOT NULL DEFAULT '',
...
KEY idx_wpid (wpid),
KEY idx_issub_id (issub),
KEY idx_wpid_monitor_id (wpid, monitor_id),
KEY idx_auto_start (dts_auto_monitoring_start),
KEY idx_auto_time_check (dts_auto_monitoring_retry_time)";
    ...
    KEY idx_monitor_id_time (monitor_id, time)';
        ...
        $need_update = ! empty( $currentVersion ) && version_compare( $currentVersion, '9.0.0.41', '<' );

        if ( $need_update && false !== get_option( 'mainwp_disableSitesChecking' ) ) {
            ...
            MainWP_Utility::update_option( 'mainwp_global_uptime_monitoring_settings', $global_settings );

            // delete_option('mainwp_disableSitesChecking'); // do not delete for compatible.
        }
    }
    ...
    public function get_monitors_to_check_uptime( $params = array() ) {
        ...
        $and_active = ' AND ( mo.active = 1 OR ( mo.active = 2 AND 1 = ' . $glo_active . ') ) ';
        ...
        $and_retry_run .= ' ( mo.dts_auto_monitoring_retry_time + mo.retry_interval * 60 <= ' . intval( $local_timestamp ) . ' ) '; // To ensure the ping request is not completed.
     ...
     * @return int
     */
    public function count_individual_enabled_monitoring() {
    ...
    public function count_busy_main_round_check( $global, $lasttime_counter ) {
        ...
        $where_round_run  = ' AND ( mo.active = 1 OR ( mo.active = 2 AND 1 = ' . intval( $glo_active ) . ') ) '; // make sure active monitor.
    ...
    public function get_sql_monitor( $params ) {
        ...
        if ( 'base_view' === $view || 'ping_view' === $view || 'monitor_view' === $view ) {
    ...
    public function delete_monitor( $params ) {
        ...
        if ( ! empty( $params['monitor_id'] ) ) {
            ...
            $monitor = $this->get_monitor_by( false, 'monitor_id', $params['monitor_id'] );

            if ( empty( $monitor ) ) {
                return false;
            }

            if ( $this->wpdb->query( $this->wpdb->prepare( 'DELETE FROM ' . $this->table_name( 'monitors' ) . ' WHERE monitor_id=%d', $params['monitor_id'] ) ) ) {
                $this->delete_heartbeat( $monitor->monitor_id );
                return true;
            }
        }
    ...
    public function get_last_site_incidents_stats( $siteid ) {
        $sql = $this->wpdb->prepare(
            'SELECT ' .
            ' ( SELECT count(*) FROM ' . $this->table_name( 'monitor_heartbeat' ) . ' he24 WHERE ( he24.time >= NOW() - INTERVAL 1 DAY ) AND he24.monitor_id = he.monitor_id AND he24.status = 0 ) AS total24,' .
            ' ( SELECT count(*) FROM ' . $this->table_name( 'monitor_heartbeat' ) . ' he7 WHERE ( he7.time >= NOW() - INTERVAL 7 DAY ) AND he7.monitor_id = he.monitor_id AND he7.status = 0 ) AS total7,' .
            ' ( SELECT count(*) FROM ' . $this->table_name( 'monitor_heartbeat' ) . ' he30 WHERE ( he30.time >= NOW() - INTERVAL 30 DAY ) AND he30.monitor_id = he.monitor_id AND he30.status = 0 ) AS total30 ' .
    ...
    public function get_site_incidents_stats( $siteid, $params = array() ) {
        ...
        $sql = $this->wpdb->prepare(
            'SELECT ' .
            ' ( SELECT count(*) FROM ' . $this->table_name( 'monitor_heartbeat' ) . ' inct WHERE inct.time > "' . $this->escape( $start ) . '" AND inct.time <= "' . $this->escape( $end ) . '" AND inct.monitor_id = he.monitor_id AND inct.status = 0 ) AS count_incidents' .
            ' FROM ' . $this->table_name( 'monitors' ) . ' mo ' .
            ' LEFT JOIN ' . $this->table_name( 'monitor_heartbeat' ) . ' he ' .
            ' ON mo.monitor_id = he.monitor_id ' .
            ' WHERE mo.wpid = %d LIMIT 1',
     ...
     * @return object|null Database query results or null on failure.
     */
    public function get_site_response_time_days_stats( $siteid, $params = array() ) {

        $start = isset( $params['start'] ) ? $params['start'] . ' 00:00:00' : gmdate( 'Y-m-d 00:00:00', time() - 7 * DAY_IN_SECONDS );
        $end   = isset( $params['end'] ) ? $params['end'] . ' 23:59:59' : gmdate( 'Y-m-d 23:59:59', time() );

        $sql = $this->wpdb->prepare(
            'SELECT DATE( he.time ) AS resp_date, SUM(he.ping_ms) as resp_ms FROM ' . $this->table_name( 'monitors' ) . ' mo ' .
            ' LEFT JOIN ' . $this->table_name( 'monitor_heartbeat' ) . ' he ' .
            ' ON mo.monitor_id = he.monitor_id ' .
