    ...
    public function prepare_items( $optimize = true ) { // phpcs:ignore -- NOSONAR -Current complexity is the only way to achieve desired results, pull request solutions appreciated.
        ...
        if ( '' !== $site_status && 'all' !== $site_status ) {
            if ( 'connected' === $site_status ) {
                ...
                $where = MainWP_DB_Common::instance()->get_sql_where_for_phpver( 'owp_phpversion', '<', '8.0.0.0' ); // NOSONAR - no IP.
                if ( $is_not ) {
                    $where = MainWP_DB_Common::instance()->get_sql_where_for_phpver( 'owp_phpversion', '>=', '8.0.0.0' ); // NOSONAR - no IP.
                }
            } elseif ( 'phpver81' === $site_status ) {
                $where = MainWP_DB_Common::instance()->get_sql_where_for_phpver( 'owp_phpversion', '<', '8.1.0.0' ); // NOSONAR - no IP.
                if ( $is_not ) {
                    $where = MainWP_DB_Common::instance()->get_sql_where_for_phpver( 'owp_phpversion', '>=', '8.1.0.0' ); // NOSONAR - no IP.
                }
            } elseif ( 'phpver82' === $site_status ) {
                $where = MainWP_DB_Common::instance()->get_sql_where_for_phpver( 'owp_phpversion', '<', '8.2.0.0' ); // NOSONAR - no IP.
                if ( $is_not ) {
                    $where = MainWP_DB_Common::instance()->get_sql_where_for_phpver( 'owp_phpversion', '>=', '8.2.0.0' ); // NOSONAR - no IP.
                }
            } elseif ( 'phpver83' === $site_status ) {
                $where = MainWP_DB_Common::instance()->get_sql_where_for_phpver( 'owp_phpversion', '<', '8.3.0.0' ); // NOSONAR - no IP.
                if ( $is_not ) {
                    $where = MainWP_DB_Common::instance()->get_sql_where_for_phpver( 'owp_phpversion', '>=', '8.3.0.0' ); // NOSONAR - no IP.
                }
            } elseif ( 'phpver84' === $site_status ) {
                $where = MainWP_DB_Common::instance()->get_sql_where_for_phpver( 'owp_phpversion', '<', '8.4.0.0' ); // NOSONAR - no IP.
                if ( $is_not ) {
                    $where = MainWP_DB_Common::instance()->get_sql_where_for_phpver( 'owp_phpversion', '>=', '8.4.0.0' ); // NOSONAR - no IP.
                }
        ...
        if ( in_array( $site_status, array( 'phpver8', 'phpver81', 'phpver82', 'phpver83', 'phpver84' ) ) ) {
            $params['_where_php_ver'] = 1;
        }
        ...
        if ( $this->items ) {
            foreach ( $this->items as $website ) {
                ...
                if ( 80 <= $hval && empty( $critical ) ) {
                    $site_health = '<a href="' . MainWP_Site_Open::get_open_site_admin_link( $website['id'] ) . '&location=' . esc_attr( base64_encode( 'site-health.php' ) ) . '" target="_blank" class="open_newwindow_wpadmin ui mini grey icon basic button" data-tooltip="' . esc_attr__( 'Health Score: ', 'mainwp' ) . $hval . '" data-position="left center" data-inverted=""><i class="heartbeat green icon"></i></a>'; // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions -- base64_encode used for http encoding compatible.
                } else {
    ...
    protected function single_row_columns( $website, $good_health = false ) { // phpcs:ignore -- NOSONAR - complex function. Current complexity is the only way to achieve desired results, pull request solutions appreciated.
        ...
        foreach ( $columns as $column_name => $column_display_name ) {
            ...
            <?php } elseif ( 'site_health' === $column_name ) { ?>
                <td class="collapsing mainwp-site-health-cell"><a class="open_newwindow_wpadmin" href="<?php MainWP_Site_Open::get_open_site_admin_link( $website['id'], true ); ?>&location=<?php echo esc_attr( base64_encode( 'site-health.php' ) ); // phpcs:ignore -- base64_encode used for http encoding compatible. ?>" target="_blank"><span class="ui <?php echo esc_attr( $h_color ); ?> empty circular label"></span></a> <?php echo esc_html( $h_text ); ?></td>
            <?php } elseif ( 'status_code' === $column_name ) { ?>
