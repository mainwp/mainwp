...
class MainWP_Utility {
	...
	public static function tryVisit( $url, $verifyCertificate = null, $http_user = null, $http_pass = null, $sslVersion = 0, $forceUseIPv4 = null ) {
		...
		if ( $verifyCertificate !== null ) {
			if ( $verifyCertificate == 1 ) {
				$ssl_verifyhost = true;
			} elseif ( $verifyCertificate == 2 ) { // use global setting
		...
		if ( $forceUseIPv4 !== null ) {
			if ( $forceUseIPv4 == 1 ) {
				$force_use_ipv4 = true;
			} elseif ( $forceUseIPv4 == 2 ) { // use global setting
	...
	static function fetchUrlAuthed( &$website, $what, $params = null, $checkConstraints = false, $pForceFetch = false,
		...
		if ( ! is_array( $params ) ) {
		 ...
		 $others = array(
			 ...
			 'upgrade'        => ( $what == 'upgradeplugintheme' || $what == 'upgrade' || $what == 'upgradetranslation' ),
		 );
		 ...
		 if ( $what == 'stats' || ( $what == 'upgradeplugintheme' && isset( $params['type'] ) ) ) {
			 ...
			 } elseif ( $what == 'upgradeplugintheme' ) {
	...
	static function _fetchUrl( &$website, $url, $postdata, $checkConstraints = false, $pForceFetch = false,
		...
		if ( ( $data === false ) && ( $http_status == 0 ) ) {
			...
			return $information;
		} elseif ( $http_status == 200 && ! empty( $err ) ) { // unexpected http error
			throw new MainWP_Exception( 'HTTPERROR', $err );
		} elseif ( $raw_response ) {
...
EOT;
	...
	public static function getWebsitesAutomaticUpdateTime() {
		...
		} elseif ( MainWP_DB::Instance()->getWebsitesCountWhereDtsAutomaticSyncSmallerThenStart() > 0 || MainWP_DB::Instance()->getWebsitesCheckUpdatesCount() > 0 ) {
	...
	public static function getCurrentArchiveExtension( $website = false, $task = false ) {
		...
		if ( $task != false ) {
			if ( $task->archiveFormat == 'global' ) {
				$useGlobal = true;
				$useSite   = false;
			} elseif ( $task->archiveFormat == '' || $task->archiveFormat == 'site' ) {
				$useGlobal = false;
				$useSite   = true;
			} else {
	...
	public static function get_favico_url( $website ) {
		...
		if ( ! empty( $favi ) ) {
			...
			} elseif ( ( strpos( $favi, '//' ) === 0 ) || ( strpos( $favi, 'http' ) === 0 ) ) {
				$faviurl = $favi;
			} else {
