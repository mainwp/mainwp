...
class Api_Backups_3rd_Party {
	...
	public static function action_notifications() {
		...
		} elseif ( isset( $_GET['backup'] ) && ( $_GET['backup'] === 'false' ) ) {
	...
	public static function render_mainwp_backups_page() {
		?>
			...
			<?php endif; ?>
			<?php self::action_notifications(); ?>
			<?php self::render_api_backups_table( '3rd-party-api-backups' ); ?>
	 ...
	 *
	 * @param string $backup_api The backup API being used to render the table row.
	 *
	 * @return void
	 */
	public static function render_api_backups_table( $backup_api ) {
		...
		<table id="mainwp-3rd-party-backups-table" class="ui mainwp-api-backup-table table" style="width:100%">
			...
			<tbody>
				<?php while ( $websites && ( $website = Api_Backups_Helper::fetch_object( $websites ) ) ) { ?>
					...
					?>
					<tr website-id="<?php echo intval( $website->id ); ?>" provider-name="<?php echo $api_provider; ?>">
						<td class="check-column">
							<?php if ( $api_provider != 'None' ) : ?>
								<span class="ui checkbox">
						...
						<td>
							<a href="<?php echo 'admin.php?page=SiteOpen&newWindow=yes&websiteid=' . intval( $website->id ); ?>&_opennonce=<?php echo wp_create_nonce( 'mainwp-admin-nonce' ); ?>" target="_blank">
						...
						<td class="last-backup-date" ><?php echo ( $last_backup ) ? $last_backup : esc_html__( 'Awaiting first backup', 'mainwp' ); ?></td>
						<td>
							<i class="ui notched circle loading icon" style="display:none;"></i>
							<div class="ui left pointing dropdown icon mini basic green button" style="z-index: 999">
								<a href="javascript:void(0)"><i class="ellipsis horizontal icon"></i></a>
								<div class="menu">
									<?php if ( $api_provider === 'cPanel' ) : ?>
									...
									<?php elseif ( $api_provider != 'None' && $api_provider != 'cPanel' ) : ?>
				...
				<?php } ?>
				<?php Api_Backups_Helper::free_result( $websites ); ?>
			</tbody>
		</table>
	...
	<?php } // END render_api_backups_table().
	...
	public function render_api_backups_site( $website ) {
		...
		if ( isset( $site_available_backups[ 'mainwp_3rd_party_' . $backup_api . '_available_backups' ] ) ) {
		...
		if ( empty( $backup_api ) ) {
			...
			<?php } else { ?>
						...
						<div class="ui <?php echo esc_html( $columns ); ?> column row">
							...
							<div class="right aligned middle aligned column">
								...
								if ( 'digitalocean' === $backup_api ) {
									...
									$digitalocean_droplet_id = Api_Backups_Helper::get_website_options( $website, array( 'mainwp_3rd_party_instance_id', ) );
								...
								?>
									<a href="https://cloud.digitalocean.com/<?php echo $droplet_id; ?>/backup" target="_blank" class="ui mini button">
								...
								if ( 'linode' === $backup_api ) {
									// Grab Linode instance id.
									$linode_instance_id = Api_Backups_Helper::get_website_options( $website, array( 'mainwp_3rd_party_instance_id', ) );
								...
								?>
									...
									<a href="https://cloud.linode.com/linodes/<?php echo $instance_id; ?>/backup" target="_blank" class="ui mini button">
										<i class="external icon"></i>
								...
								?>
									...
									<a href="https://platform.cloudways.com/apps/<?php echo $app_id; ?>/restore" target="_blank" class="ui mini button">
										<i class="external icon"></i>
								...
								?>
									<a href="https://my.vultr.com/subs/?id=<?php echo $instance_id; ?>#subssnapshots" target="_blank" class="ui mini button">
										<i class="external icon"></i>
				...
				<div class="ui segment">
					...
					<?php if ( 'kinsta' === $backup_api ) {

						if ( is_object( $available_backups ) ) {
							$environment = $available_backups->environment->display_name;
					...
					?>
						<table id="mainwp-siteid-<?php echo intval( $website['id'] ); ?>-table" class="ui mainwp-api-backup-table table" style="width:100%">
							...
							<tbody>
								<?php foreach ( $available_backups as $backup ) { ?>
									<?php
										$backup_date = date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( $backup->value->createdAt->value ) );
					...
					// Check if valid JSON or not. If not, then it's a string - do not decode it.
					if ( isset( $available_backups_manual_list[ 'mainwp_3rd_party_' . $backup_api . '_manual_backups' ] ) ) {
					...
					// Check if valid JSON or not. If not, then it's a string - do not decode it.
					if ( isset( $mainwp_3rd_party_cpanel_manual_database_backups[ 'mainwp_3rd_party_' . $backup_api . '_manual_database_backups' ] ) ) {
					...
					<div class="ui bottom attached active tab" data-tab="cpanel-native">
						...
						<table id="mainwp-siteid-<?php echo intval( $website['id'] ); ?>-table" class="ui mainwp-api-backup-table table" style="width:100%">
								...
								<?php foreach ( $available_backups_manual_list as $backup ) { ?>
									...
									$backup_date       = get_date_from_gmt( date( 'Y-m-d H:i:s', $my_unix_timestamp ), get_option( 'date_format' ) . ' ' . get_option( 'time_format' ) );
									?>
									<tr>
				...
				<?php } ?>
					...
					if ( isset( $available_backups->application_backup_exists ) && $available_backups->application_backup_exists === true ) {
						...
						<table id="mainwp-siteid-<?php echo intval( $website['id'] ); ?>-table" class="ui mainwp-api-backup-table table" style="width:100%">
							...
							<?php foreach ( $available_backups as $backup ) { ?>
								<tr>
									...
									<td><?php echo esc_html( date( 'm-d-Y ( h:i:s )', strtotime( $backup ) ) ); ?></td>
									<td></td>
									<td>
						...
						?>
							<table id="mainwp-siteid-<?php echo intval( $website['id'] ); ?>-table" class="ui mainwp-api-backup-table table" style="width:100%">
								...
								<?php foreach ( $available_snapshots_list as $backup ) { ?>
									<tr>
										<td><i class="ui notched circle loading icon" style="display:none;"></i> <?php esc_html_e( $backup->description ); ?></td>
										...
										<td><?php esc_html_e( $backup->status ); ?></td>
										<td></td>
										<td>
								...
								<?php foreach ( $available_backups_list as $backup ) { ?>
									<tr>
										<td><i class="ui notched circle loading icon" style="display:none;"></i> <?php esc_html_e( $backup->description ); ?></td>
										<td><?php echo esc_html( Api_Backups_Utility::human_filesize( $backup->size ) ); ?></td>
										<td><?php echo esc_html( 'N/A', 'mainwp' ); ?></td>
										<td><?php echo esc_html( Api_Backups_Utility::format_timestamp( strtotime( $backup->date_created ) ) ); ?></td>
										<td><?php esc_html_e( $backup->status ); ?></td>
										<td></td>
										<td>
							...
							<table id="mainwp-siteid-<?php echo intval( $website['id'] ); ?>-table" class="ui mainwp-api-backup-table table" style="width:100%">
								...
								<?php foreach ( $available_backups_automatic_list as $backup ) { ?>
									<tr>
										...
										<td class="operation_status"><?php esc_html_e( $backup->status ); ?></td>
										<td></td>
										<td></td>
										<td>
											<a class="mainwp_3rd_party_api_<?php echo esc_attr( $backup_api ); ?>_action_restore_backup ui mini icon button"
						...
						<table id="mainwp-siteid-<?php echo intval( $website['id'] ); ?>-table" class="ui mainwp-api-backup-table table" style="width:100%">
							...
							<?php foreach ( $available_backups as $backup ) { ?>
								<tr>
									...
									<td><?php esc_html_e( Api_Backups_Utility::format_timestamp( strtotime( $backup->created_at ) ) ); ?></td>
									<td></td>
									<td>
										<a class="mainwp_3rd_party_api_<?php echo esc_attr( $backup_api ); ?>_action_restore_backup ui mini icon button"
											website_id="<?php echo intval( $website['id'] ); ?>"
	...
	public static function call_cloudways_api( $method, $url, $accessToken, $post = array() ) {
		...
		Api_Backups_Utility::log_debug( '[' . $httpcode . ']::' . $baseURL . $url );
		Api_Backups_Utility::log_debug( $output );
	...
	public static function fetch_cloudways_access_token() {
		...
		$api_key       = self::get_cloudways_api_key();

		// Fetch Access Token
		$tokenResponse = self::call_cloudways_api(
			'POST',
			'/oauth/access_token',
			null,
			array(
	...
	public static function fetch_cloudways_server_list() {
		// Grab access token.
		$accessToken = self::fetch_cloudways_access_token();

		// Fetch Server List
		$serverList = (array) self::call_cloudways_api( 'GET', '/server', $accessToken );
	...
	public function ajax_cloudways_action_update_ids() {
		...
		self::cloudways_action_update_ids();
	}
		...
		foreach ( $server_list as $server ) {
			foreach ( $server->apps as $app ) {
				if ( in_array( $app->cname, $child_sites ) ) {
					...
					// Check if app fqdn name matches any Child Site domain name, if so, save the server & app ID to that Child Site's options table.
					if ( in_array( $app->app_fqdn, $child_sites ) ) {
	...
	public function ajax_cloudways_action_backup() {
		...
		self::cloudways_action_create_backup();
	}
	 ...
	 * @return void
	 */
	public static function cloudways_action_create_backup( $website_id = '', $return = false ) {

		$website_id = isset( $_POST['website_id'] ) ? intval( $_POST['website_id'] ) : $website_id;
		...
		$api_response = (array) self::call_cloudways_api( 'POST', '/app/manage/takeBackup', $accessToken, $data );
		...
		if ( ! $api_response['status'] === true ) {
			...
			$backup_operation = (array) self::call_cloudways_api( 'GET', '/operation/' . $api_response['operation_id'], $accessToken );
			$backup_operation = json_encode( $backup_operation['operation'] );
			...
			// self::cloudways_action_poll_backup_operation( $website_id  );
		}
	...
	public static function cloudways_action_poll_backup_operation( $website_id ) {
		...
		$operation_polling       = self::call_cloudways_api( 'GET', '/operation/' . $operation_id, $accessToken );
		...
		if ( ! $backup_polling_response->is_completed === true ) {
			sleep( 30 );
			self::cloudways_action_poll_backup_operation( $website_id );
		} else {
			// Save current backup operation.
			Api_Backups_Helper::update_website_option( $website_id, 'mainwp_3rd_party_cloudways_backup_operation', json_encode( $backup_polling_response ) );
	...
	public static function cloudways_action_refresh_available_backups() {
		...
		$backup_operation = self::call_cloudways_api( 'GET', '/app/manage/backup?server_id=' . $server_id . '&app_id=' . $app_id, $accessToken );
		...
		if ( is_admin() && defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			if ( ! $operation->is_completed === true ) {
				wp_die( 'false' );
			} else {
				// Return success.
				wp_send_json( 'true' );
	...
	public static function call_vultr_api( $method, $url, $accessToken, $data = null, $not_die = false ) {
		...
		Api_Backups_Utility::log_debug( '[' . $httpCode . ']::' . $Baseurl . $url );
		Api_Backups_Utility::log_debug( $resp );
	...
	public function ajax_vultr_action_individual_create_snapshot() {
		...
		self::vultr_action_create_snapshot();
	}
	 ...
	 * @return void
	 */
	public static function vultr_action_create_snapshot( $website_id = '', $backup_api = '', $return = false ) {

		// Grab $_POST['data'] if set else use args.
		$website_id = isset( $_POST['website_id'] ) ? $_POST['website_id'] : $website_id;
		$backup_api = isset( $_POST['backup_api'] ) ? $_POST['backup_api'] : $backup_api;
		...
		$url       = ! empty( $website['url'] ) ? urlencode( $website['url'] ) : '';
		...
		// Send Payload & create backup.
		$snapshot_response = self::call_vultr_api( 'POST', '/snapshots', $accessToken, $snapshot_data );
		$snapshot_id       = $snapshot_response['response']->snapshot->id;

		$snapshot_list   = json_decode( $snapshot_list );
		$snapshot_list[] = $snapshot_id;
		$snapshot_list   = json_encode( $snapshot_list );
	...
	public function vultr_action_refresh_available_backups() {
		...
		$available_backups            = json_encode( $available_backups );
	...
	public function ajax_vultr_action_restore_backup() {
		...
		self::vultr_action_restore_backup();
	}
	...
	public function vultr_action_restore_backup() {
		...
		$api_response = (object) self::call_vultr_api( 'POST', '/instances/' . $instance_id . '/restore', $accessToken, $backup_data );
	...
	public function vultr_action_delete_backup() {
		...
		$api_response = (object) self::call_vultr_api( 'DELETE', '/snapshots/' . $snapshot_id, $accessToken );
		...
		if ( ( $key = array_search( $snapshot_id, $snapshot_list ) ) !== false ) {
	 ...
	 * @param string $url relative URL for the call
	 * @param string $accessToken Access token generated using OAuth Call
	 * @param mixed  $data Json encoded array - Optional post data for the call
	...
	public static function call_gridpane_api( $method, $url, $accessToken, $backup_data = array() ) {
		...
		Api_Backups_Utility::log_debug( '[' . $httpCode . ']::' . $Baseurl . $url );
		Api_Backups_Utility::log_debug( $response );
	...
	public static function gridpane_get_sites_list( $sites_count ) {
		...
		// Grab Sites List.
		$grid_response         = (array) self::call_gridpane_api( 'GET', '/site?per_page=' . $sites_count, $accessToken );
	...
	public function ajax_gridpane_action_update_ids() {
		...
		self::gridpane_action_update_ids();
	}
	...
	public function ajax_gridpane_action_individual_create_backup() {
		...
		self::gridpane_action_create_backup();
	}
	 ...
	 * @return void
	 */
	public static function gridpane_action_create_backup( $website_id = '', $return = false ) {
		...
		// Send Payload & create backup.
		$backup_response = self::call_gridpane_api( 'POST', '/backups/' . $site_id, $accessToken, $backup_data );
	...
	public function gridpane_action_refresh_available_backups() {
		...
		// Refresh backups.
		self::call_gridpane_api( 'GET', '/backups/refresh/' . $site_id, $accessToken );

		// Send Payload & grab backup list.
		$api_response = json_decode( self::call_gridpane_api( 'GET', '/backups/original/' . $site_id, $accessToken ) );

		$available_backups = '';
		if ( $api_response && ! empty( $api_response->backups ) && ! empty( $api_response->backups->local ) ) {
			$available_backups = json_encode( $api_response->backups->local );
		}
	...
	public function gridpane_action_restore_backup() {
		...
		// Send Payload & list backups. backups/restore/1038'
		$api_response = json_decode( self::call_gridpane_api( 'POST', '/backups/restore/' . $site_id, $accessToken, $backup_data ) );
	...
	public function gridpane_action_delete_backup() {
		...
		// Send Payload & delete backup.
		$api_response = json_decode( self::call_gridpane_api( 'PUT', '/backups/purge/' . $site_id, $accessToken, $backup_data ) );
	...
	public function ajax_linode_action_individual_create_backup() {
		...
		self::linode_action_create_backup();
		die();
	}
	 ...
	 * @return void
	 */
	public static function linode_action_create_backup( $website_id = '', $return = false ) {
		...
		$accessToken = self::get_linode_api_key();

		// Send Payload & create backup.
		$linode_response = (array) self::call_linode_api( 'POST', '/instances/' . $instance_id . '/backups', $accessToken );
	...
	public function linode_action_refresh_available_backups() {
		...
		$api_response = (object) self::call_linode_api( 'GET', '/instances/' . $instance_id . '/backups', $accessToken );
	...
	public function linode_action_restore_backup() {
		...
		$restore_response = (array) self::call_linode_api( 'POST', '/instances/' . $instance_id . '/backups/' . $backup_id . '/restore', $accessToken, $backup_data );
		$restore_response = (array) json_decode( $restore_response['0'] );

		sleep( 20 );
		self::linode_action_linode_status( $website_id );
	}
	 ...
	 *
	 * @param $website_id int Child Site ID.
	 * @param $backup_api string Backup API.
	 *
	 * @return void
	 */
	public static function linode_action_linode_status( $website_id ) {
		...
		$linode_instance_response = (array) self::call_linode_api( 'GET', '/instances/' . $instance_id, $accessToken );
		...
		switch ( $linode_instance_status ) {
			...
			case 'offline':
				// Starting server.
				self::linode_action_linode_start( $instance_id );
				break;
		}
	}
	 ...
	 *
	 * @param $instance_id int Linode Instance ID.
	 * @param $website_id int Child Site ID.
	 * @param $backup_api string Backup API.
	 *
	 * @return void
	...
	public static function linode_action_linode_start( $instance_id ) {
		...
		$linode_boot_response = (array) self::call_linode_api( 'POST', '/instances/' . $instance_id . '/boot', $accessToken );
	...
	public function linode_action_cancel_backups() {
		...
		$api_response = (object) self::call_linode_api( 'POST', '/instances/' . $instance_id . '/backups/cancel', $accessToken );
	...
	public function ajax_digitalocean_action_create_backup() {
		...
		self::digitalocean_action_create_backup();
	}
	 ...
	 * @return void
	 */
	public static function digitalocean_action_create_backup( $website_id = '', $return = false ) {
		...
		// Grab Access token.
		$accessToken = self::get_digitalocean_api_key();
		...
		// Send Payload & create backup.
		$api_response = self::call_digitalocean_api( 'POST', '/droplets/' . $droplet_id . '/actions', $accessToken, $backup_data );
	...
	public function digitalocean_action_refresh_available_backups() {
		...
		// Send Payload & create backup.
		$api_response = self::call_digitalocean_api( 'GET', '/droplets/' . $droplet_id . '/snapshots', $accessToken );
	...
	public static function digitalocean_action_delete_backup() {
		...
		// $snapshot_id = '121907743';

		// Grab Access token.
		$accessToken = self::get_digitalocean_api_key();

		// Send Payload.
		$api_response = self::call_digitalocean_api( 'DELETE', '/snapshots/' . $snapshot_id, $accessToken );
	...
	public static function get_plesk_home_dir() {
		...
		$plesk_authentication_credentials = self::get_plesk_authentication_credentials();
		...
		$api_response = self::call_plesk_api( 'GET', '/api/modules/wp-toolkit/v1/installations?installationsIds%5B%5D=' . $plesk_installation_id, $plesk_baseurl, $plesk_api_key );
	...
	public static function get_plesk_authentication_credentials( $website_id = null ) {
		...
		if ( 'on' === $mainwp_enable_plesk_individual ) {
			...
			// Grab Plesk password.
			$plesk_api_key = self::get_plesk_api_key();
	...
	public function ajax_plesk_action_refresh_available_backups() {
		...
		self::plesk_action_refresh_available_backups();
		die();
	}
	...
	public function ajax_plesk_action_create_backup() {
		...
		self::plesk_action_create_backup();
		die();
	}
	...
	public function ajax_plesk_action_restore_backup() {
		...
		self::plesk_action_restore_backup();
		die();
	}
	...
	public function ajax_plesk_action_delete_backup() {
		...
		self::plesk_action_delete_backup();
		die();
	}
	...
	public static function plesk_action_refresh_available_backups() {
		...
		// Grab backup meta.
		$api_response = self::call_plesk_api( 'GET', '/api/modules/wp-toolkit/v1/installations/' . $plesk_installation_id . '/backups/meta', $plesk_baseurl, $plesk_api_key );
	 ...
	 * @return array
	 */
	public static function plesk_action_create_backup( $return = false, $website_id = null ) {

		// Authenticate plesk account.
		$plesk_authentication_credentials = self::get_plesk_authentication_credentials( $website_id );
		$plesk_baseurl                    = $plesk_authentication_credentials[0]['plesk_baseurl'];
		$plesk_api_key                    = $plesk_authentication_credentials[0]['plesk_api_key'];
		$plesk_installation_id            = $plesk_authentication_credentials[0]['plesk_installation_id'];
		$website_id                       = $plesk_authentication_credentials[0]['website_id'];
		...
		$api_response = self::call_plesk_api( 'POST', '/api/modules/wp-toolkit/v1/features/backups/creator', $plesk_baseurl, $plesk_api_key, $backup_data );
	...
	public static function plesk_action_restore_backup() {
		...
		$api_response = self::call_plesk_api( 'POST', '/api/modules/wp-toolkit/v1/features/backups/restorer', $plesk_baseurl, $plesk_api_key, $backup_data );
	...
	public static function plesk_action_delete_backup() {
		...
		// Send Payload & create backup.
		$api_response = self::call_plesk_api( 'DELETE', $payload, $plesk_baseurl, $plesk_api_key, $backup_data );
	...
	public static function cPanel_action_get_home_directory() {
		...
		// Send Payload.
		$api_response         = self::call_cPanel_api( 'GET', '/execute/Variables/get_user_information', $cpanel_baseurl, $cpanel_username, $cpanel_password );
	...
	public static function get_cpanel_manual_account_backups() {
		...
		// Send Payload.
		$api_response         = self::call_cPanel_api( 'GET', '/execute/Fileman/list_files?dir=/&include_mime=1&types=file', $cpanel_baseurl, $cpanel_username, $cpanel_password );
		$api_response_decoded = json_decode( $api_response['response'] );
		...
		return json_encode( $manual_backups );
	}
	...
	public static function get_cpanel_manual_database_backups( $website_id = '' ) {
		...
		// Send Payload.
		$api_response         = self::call_cPanel_api( 'GET', '/execute/Fileman/list_files?dir=' . $cpanel_db_backups_path . '&include_mime=1&types=file', $cpanel_baseurl, $cpanel_username, $cpanel_password );
		$api_response_decoded = json_decode( $api_response['response'] );
	...
	public function get_cpanel_wp_toolkit_backups( $website_id ) {
		...
		$wp_toolkit_backups = self::call_cPanel_api( 'GET', '/3rdparty/wpt/index.php/v1/installations/' . $wp_toolkit_installation_id . '/backups/meta', $cpanel_baseurl, $cpanel_username, $cpanel_password );
		if ( $wp_toolkit_backups['status'] === 'true' ) {
	...
	public function ajax_cpanel_action_restore_backup() {
		...
		self::cpanel_action_restore_backup();
		die();
	}
	...
	public function ajax_cpanel_action_restore_database_backup() {
		...
		self::cpanel_action_restore_database_backup();
		die();
	}
	...
	public function ajax_cpanel_action_restore_manual_backup() {
		...
		self::cpanel_action_restore_manual_backup();
		die();
	}
	...
	public function ajax_cpanel_action_refresh_available_backups() {
		...
		self::cpanel_action_refresh_available_backups();
		die();
	}
	...
	public function ajax_cpanel_action_create_manual_backup() {
		...
		self::cpanel_action_create_manual_backup();
		die();
	}
	...
	public function ajax_cpanel_action_create_wptk_backup() {
		...
		self::cpanel_action_create_wptk_backup();
		die();
	}
	...
	public function ajax_cpanel_action_restore_wptk_backup() {
		...
		self::cpanel_action_restore_wptk_backup();
		die();
	}
	...
	public function ajax_cpanel_action_delete_wptk_backup() {
		...
		self::cpanel_action_delete_wptk_backup();
		die();
	}
	...
	public static function ajax_cpanel_action_create_full_backup() {
		...
		if ( is_admin() && defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			...
			// Create file backup.
			$file_backup_response = self::cpanel_action_create_manual_backup( true );
			...
			if ( empty( $errors ) ) {
				...
				$database_backup_response = self::ajax_cpanel_action_create_database_backup( true, $website_id );

				// If no errors, & WP-Toolikit is enabled create WP-Toolkit backup or return true.
				if ( $database_backup_response['result'] === 'GOOD' ) {
	 ...
	 * @return void | array
	 */
	public static function ajax_cpanel_action_create_database_backup( $return = false, $website_id = null ) {
	 ...
	 * @return array
	 */
	public static function cpanel_action_create_manual_backup( $return = false, $website_id = null ) {

		// Authenticate cPanel account.
		$cpanel_authentication_credentials = self::get_cpanel_authentication_credentials( $website_id );
		$website_id                        = $cpanel_authentication_credentials[0]['website_id'];

		// Send Payload & create backup.
		$api_response     = self::call_cPanel_api( 'GET', '/execute/Backup/fullbackup_to_homedir', $cpanel_authentication_credentials[0]['cpanel_baseurl'], $cpanel_authentication_credentials[0]['cpanel_username'], $cpanel_authentication_credentials[0]['cpanel_password'] );
		$response_decoded = is_array( $api_response ) && ! empty( $api_response['response'] ) ? json_decode( $api_response['response'] ) : '';
	 ...
	 * @return array|void
	 */
	public static function cpanel_action_create_wptk_backup( $return = false ) {
		...
		// Grab list of WP Toolkit installations.
		$wp_toolkit_backup_response         = self::call_cPanel_api_json( 'POST', '/3rdparty/wpt/index.php/v1/features/backups/creator', $cpanel_baseurl, $cpanel_username, $cpanel_password, $backup_data );
	 ...
	 * @return array|void
	 */
	public static function cpanel_action_restore_wptk_backup( $return = false ) {
		...
		// Grab list of WP Toolkit installations.
		$wp_toolkit_backup_response         = self::call_cPanel_api_json( 'POST', '/3rdparty/wpt/index.php/v1/features/backups/restorer', $cpanel_baseurl, $cpanel_username, $cpanel_password, $backup_data );
	...
	public static function cpanel_action_delete_wptk_backup() {
		 ...
		 */
		$payload = '/3rdparty/wpt/index.php/v1/installations/' . $wp_toolkit_installation_id . '/backups?files%5B%5D=' . urlencode( $backup_name );

		// Send Payload & create backup.
		$api_response     = self::call_cPanel_api_json( 'DELETE', $payload, $cpanel_baseurl, $cpanel_username, $cpanel_password, $backup_data );
	...
	public function cpanel_action_refresh_available_backups() {
		...
		$api_response = self::call_cPanel_api( 'GET', '/execute/Restore/query_file_info?path=' . $cpanel_site_path . '&exists=1', $cpanel_baseurl, $cpanel_username, $cpanel_password );
		$all_backups  = $api_response['response'];
		...
		$manual_backups = self::get_cpanel_manual_account_backups();
		...
		$database_backups = self::get_cpanel_manual_database_backups( $website_id );
		...
		if ( $wp_toolkit_enabled ) {
			$wp_toolkit_backups = self::get_cpanel_wp_toolkit_backups( $website_id );
	...
	public function cpanel_action_restore_backup() {
		...
		// Send Payload & create backup.
		$api_response = self::call_cPanel_api( 'GET', '/execute/Restore/restore_file?backupID=' . $backup_id . '&path=' . $cpanel_site_path . '&overwrite=1', $cpanel_baseurl, $cpanel_username, $cpanel_password );
	...
	public function cpanel_action_restore_manual_backup() {
		...
		// Send Payload & create backup.
		$api_response     = self::call_cPanel_api( 'POST', '/execute/Backup/restore_files?backup=' . $backup_full_path . '&directory=' . $directory, $cpanel_baseurl, $cpanel_username, $cpanel_password );
		$response_decoded = json_decode( $api_response['response'] );
		$errors           = $response_decoded->errors['0'];
		...
		if ( is_admin() && defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			// Store Last Backup timestamp.
			if ( $api_response['httpCode'] === '200' && empty( $errors ) ) {
				wp_send_json( 'true' );
			} else {
				wp_die( 'false' );
			}
	...
	public function ajax_kinsta_action_refresh_available_backups() {
		...
		self::kinsta_action_refresh_available_backups();
		die();
	}
	...
	public function ajax_kinsta_action_create_backup() {
		...
		self::kinsta_action_create_backup();
		die();
	}
	...
	public function ajax_kinsta_action_delete_backup() {
		...
		self::kinsta_action_delete_backup();
		die();
	}
	...
	public function ajax_kinsta_action_restore_backup() {
		...
		self::kinsta_action_restore_backup();
		die();
	}
	...
	public static function kinsta_action_refresh_available_backups() {
		...
		// Grab backup meta.
		$api_response = self::call_kinsta_api( 'GET', '/sites/environments/' . $kinsta_env_id . '/backups', $kinsta_baseurl, $kinsta_api_key, $action );
	 ...
	 * @return array
	 */
	public static function kinsta_action_create_backup( $return = false, $website_id = null ) {
		...
		// Send Payload & create backup.
		$api_response = self::call_kinsta_api( 'POST', '/sites/environments/' . $kinsta_env_id . '/manual-backups', $kinsta_baseurl, $kinsta_api_key, $backup_data );
	 ...
	 * @return array
	 */
	public static function kinsta_action_delete_backup( $return = false, $website_id = null ) {

		// Grab $_POST['backup_id'].
		$backup_id = $_POST['backup_id'];
		...
		// Send Payload & create backup.
		$api_response = self::call_kinsta_api( 'DELETE', $payload_url, $kinsta_baseurl, $kinsta_api_key, $action= '' );
	 ...
	 * @return array
	 */
	public static function kinsta_action_restore_backup( $return = false, $website_id = null ) {
		...
		// Send Payload & create backup.
		$api_response = self::call_kinsta_api( 'POST', $payload_url, $kinsta_baseurl, $kinsta_api_key, $action = '', $backup_data );
