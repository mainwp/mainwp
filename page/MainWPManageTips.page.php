...
class MainWPManageTips
{
    ...
    public static function render()
    {
        foreach ($_POST as $key => $val)
        {
            ...
            {
                $tip_id = $match[1];
                if (!MainWPUtility::ctype_digit($tip_id))
                {
                    continue;
                }
                ...
                if ($tip_seq == '' && $tip_content == '')
                {
                    if ($tip_id < 90000)
                    {
                        MainWPDB::Instance()->deleteTip($tip_id);
                    }
                }
                else
                {
                    if ($tip_id > 90000)
                    {
                        MainWPDB::Instance()->addTip($tip_seq, $tip_content);
                    }
                    else
                    {
                        MainWPDB::Instance()->updateTip($tip_id, $tip_seq, $tip_content);
                    }
                }
            }
        }
    ...
    <div id="mainwp_tips" class="mainwp_popup">
        ...
        <div id="mainwp_tips_title" class="mainwp_popup_title">Tip #<span id="mainwp_tips_current_label"><?php echo $currTip; ?></span></div>
