...
class MainWP_API_Settings {
	...
	public static function render() {
		MainWP_API_Settings_View::render();
	}

	public static function testAndSaveLogin( $username, $password ) {
		if ( ( $username == '' ) && ( $password == '' ) ) {
			MainWP_Utility::update_option( 'mainwp_api_username', $username );
			MainWP_Utility::update_option( 'mainwp_api_password', MainWP_Utility::encrypt( $password, 'MainWPAPI' ) );
		...
		try {
			$output['api_status'] = MainWP_API_Settings::testAPIs( 'main', true, $username, $password, false, true );
		} catch ( Exception $e ) {
			if ( $e->getMessage() == 'MAX_ACCOUNTS' ) {
				$output['api_status'] = MAINWP_API_INVALID;
				$output['error']      = MainWP_API_Settings_View::maximumInstallationsReached();
			} else {
				$parseError           = false;
		...
		MainWP_API_Settings::testAPIs( null, true, $username, $password, false, true );
	...
	public static function refresh() {
		...
		try {
			$output['api_status'] = MainWP_API_Settings::testAPIs( 'main', true, null, null, false, true );
		} catch ( Exception $e ) {
			if ( $e->getMessage() == 'MAX_ACCOUNTS' ) {
				$output['api_status'] = MAINWP_API_INVALID;
				$output['error']      = MainWP_API_Settings_View::maximumInstallationsReached();
	...
	public static function testAPIs( $pAPI = null, $forceRequest = false, $username = null, $password = null, $pIgnoreLastCheckTime = false, $saveAnyway = false ) {
		...
		if ( $forceRequest ||
		     ...
		     ( time() - $requestsDB['lastRequest'] > 48 * 60 * 60 )
		) { //Polls every 2days
			//init requests
			...
			if ( $pAPI == null ) {
				...
				foreach ( $requests as $api => $current ) {
					...
					$responseArray    = MainWP_Utility::http_post( $request, 'mainwp.com', '/versioncontrol/rqst.php', 80, $api, ( $forceRequest && ! $saveAnyway ) );
				...
				if ( $activated_info && is_array( $activated_info ) && isset( $activated_info['activated_key'] ) && ! empty( $activated_info['api_key'] ) ) {
				...
				} else { //If it was forced or the API is not yet fetched or (invalid && last fetched later then 10 minutes ago)
					if ( $forceRequest || ! isset( $requests[ $pAPI ] ) || ( $requests[ $pAPI ] == MAINWP_API_INVALID && ( $pIgnoreLastCheckTime || ( ! isset( $lastRequests[ $pAPI ] ) || ( ( time() - $lastRequests[ $pAPI ] ) > ( 10 * 60 ) ) ) ) ) ) {
						...
						$responseArray     = MainWP_Utility::http_post( $request, 'mainwp.com', '/versioncontrol/rqst.php', 80, $pAPI, ( $forceRequest && ! $saveAnyway ) );
						$jsonDecodedResp   = json_decode( $responseArray[1], true );
			...
			$requestsDB = array(
			'lastRequest'   => ( $pAPI != null ? ( isset( $requests['lastRequest'] ) ? $requests['lastRequest'] : '' ) : time() ),
			                     'requests'      => base64_encode( serialize( $requests ) ),
			                     'lastRequests'  => base64_encode( serialize( $lastRequests ) ),
			                     'maxOccurences' => base64_encode( serialize( $maxOccurences ) ),
			);
			MainWP_Utility::update_option( 'mainwp_requests', $requestsDB );
		}
	...
	public static function checkUpgrade() {
		...
		if ( $slugs != '' ) {
			...
			$password = MainWP_Utility::decrypt( get_option( 'mainwp_api_password' ), 'MainWPAPI' );
			try {
				$responseArray = MainWP_Utility::http_post( 'do=checkUpgradeV2&username=' . rawurlencode( $username ) . '&password=' . rawurlencode( $password ) . '&slugs=' . $slugs, 'mainwp.com', '/versioncontrol/rqst.php', true );
			} catch ( Exception $e ) {
	...
	public static function getUpgradeInformation( $pSlug ) {
		...
		try {
			$responseArray = MainWP_Utility::http_post( 'do=getUpgradeInformationV2&slugs=' . $pSlug . '&username=' . rawurlencode( $username ) . '&password=' . rawurlencode( $password ), 'mainwp.com', '/versioncontrol/rqst.php', true );
		} catch ( Exception $e ) {
			MainWP_Logger::Instance()->warning( 'An error occured when trying to reach the MainWP server: ' . $e->getMessage() );
		...
		$rslt->slug = MainWP_System::Instance()->slug;
