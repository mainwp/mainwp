					...
					for ( $i = 0; $i < count( $allPlugins ); $i ++ ) {
						...
						if ( $search_plugin_status != 'all' ) {
							if ( $plugin['active'] == 1 && $search_plugin_status !== 'active' ) {
								  continue;
							} elseif ( $plugin['active'] != 1 && $search_plugin_status !== 'inactive' ) {
								continue;
							}
						}
		...
		<?php foreach ( $plugins as $slug => $plugin ) : ?>
			...
			if ( ! empty( $search_status ) && $search_status != 'all' ) {
				if ( $search_status == 'trust' && ! in_array( $slug, $trustedPlugins ) ) {
					continue;
				} elseif ( $search_status == 'untrust' && in_array( $slug, $trustedPlugins ) ) {
					continue;
				} elseif ( $search_status == 'ignored' && ! isset( $decodedIgnoredPlugins[ $slug ] ) ) {
					continue;
				}
			}
			$esc_note = $strip_note = '';
