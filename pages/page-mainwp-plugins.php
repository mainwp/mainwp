...
class MainWP_Plugins {
    ...
    add_action( 'mainwp_help_sidebar_content', array( MainWP_Plugins::getClassName(), 'mainwp_help_content' ) ); // Hook the Help Sidebar content
  }
  ...
  public static function initMenu() {
    ...
    if ( isset( self::$subPages ) && is_array( self::$subPages ) ) {
      foreach ( self::$subPages as $subPage ) {
        ...
        MainWP_Plugins::init_left_menu(self::$subPages);
    }
    ...
    public static function initMenuSubPages() {
      ...
      <div class="wp-submenu sub-open" >
          ...
          <?php if ( mainwp_current_user_can( 'dashboard', 'install_plugins' ) ) : ?>
            ...
            <a href="<?php echo admin_url( 'admin.php?page=PluginsInstall' ); ?>" class="mainwp-submenu"><?php _e( 'Install Plugins', 'mainwp' ); ?></a>
            <?php endif; ?>
          <?php endif; ?>
          ...
          if ( isset( self::$subPages ) && is_array( self::$subPages ) ) {
            foreach ( self::$subPages as $subPage ) {
              if ( MainWP_Menu::is_disable_menu_item( 3, 'Plugins' . $subPage['slug'] ) )
                                continue;
                            ?>
              <a href="<?php echo admin_url( 'admin.php?page=Plugins' . $subPage['slug'] ); ?>" class="mainwp-submenu"><?php esc_html_e( $subPage['title'] ); ?></a>
                            <?php
                        }
                    }
                    ?>
  ...
  static function init_left_menu( $subPages = array() ) {
    MainWP_Menu::add_left_menu( array(
      ...
      array('title'  => __( 'Install Plugins', 'mainwp' ),
                'parent_key' => 'PluginsManage',
...
public static function render() {
    ...
    if ( $cachedSearch != null ) {
      if ( is_array( $cachedSearch['sites'] ) ) {
        ...
        <div id="mainwp-plugins-main-content" <?php echo ( $cachedSearch != null ) ? 'style="display: block;"' : ''; ?> >
        ...
        <div class="ui divider"></div>
          ...
          <div class="ui info message">
            ...
            <?php _e( 'A plugin needs to be Inactive in order for it to be Activated or Deleted.', 'mainwp' ); ?>
          </div>
		...
		<div class="ui mini form">
			<div class="field">
				<div class="ui input fluid">
					<input type="text" placeholder="<?php esc_attr_e( 'Containing keyword', 'mainwp' ); ?>" id="mainwp_plugin_search_by_keyword" class="text" value="<?php if ( $cachedSearch != null ) { echo esc_attr( $cachedSearch[ 'keyword' ] ); } ?>"/>
				</div>
			</div>
		</div>
  ...
  public static function renderTable( $keyword, $status, $groups, $sites ) {
    ...
    MainWP_Cache::addContext( 'Plugins', array( 'keyword' => $keyword, 'status' => $status, 'sites' => ( $sites != '' ) ? $sites : '', 'groups' => ( $groups != '' ) ? $groups : '' ) );
    ...
    if ( count( $output->plugins ) == 0 ) {
            ?>
      <div class="ui message yellow"><?php _e( 'No plugins found.', 'mainwp' ); ?></div>
            <?php
            $newOutput = ob_get_clean();

      $result = array( 'result' => $newOutput, 'bulk_actions' => $bulkActions );
      ...
      <?php foreach ( $sites as $site_id => $site_url ) : ?>
        <tr>
          ...
          <td class="center aligned">
            <?php if ( isset( $sitePlugins[$site_id] ) && isset( $sitePlugins[$site_id][$plugin_name] ) && ( $muPlugins[$plugin_name] == 0 ) ) : ?>
              <?php if ( !isset( $pluginsMainWP[$plugin_name] ) || $pluginsMainWP[$plugin_name] === 'F' ) : ?>
                <div class="ui checkbox">
              ...
              <?php elseif ( isset( $pluginsMainWP[$plugin_name] ) && $pluginsMainWP[$plugin_name] === 'T' ) : ?>
  ...
  public static function PluginsSearch_handler( $data, $website, &$output ) {
    ...
    if ( !MainWP_Utility::can_edit_website( $website ) ) {
      die( json_encode( array( 'error' => __( 'You are not allowed to edit this website.', 'mainwp' ) ) ) );
        }
  ...
  public static function action( $pAction ) {
    ...
    if ( !isset( $information['status']) || ( $information['status'] != 'SUCCESS' ) ) {
      die( json_encode( array( 'error' => __( 'Unexpected error. Please try again.', 'mainwp' ) ) ) );
        }
                  ...
                  <div class="results"></div>
                        ...
                        <script type="text/javascript">
                            jQuery( document ).ready(function () {
								...
								<?php do_action( 'mainwp-install-plugins-actions-bar-left' ); ?>
                        </div>
                    <div class="right aligned column">
                    ...
                    <?php do_action( 'mainwp-install-plugins-actions-bar-right' ); ?>
                    </div>
						</div>
					</div>
				</div>
          ...
          <div class="mainwp-search-options">
            <div class="ui info message">
                ...
                <select class="ui fluid dropdown" id="mainwp_au_plugin_status">
                  <option value="all" <?php echo ( $cachedAUSearch != null && $cachedAUSearch['plugin_status'] == 'all') ? 'selected' : ''; ?>><?php esc_html_e( 'Active and Inactive', 'mainwp' ); ?></option>
                  <option value="active" <?php echo ( $cachedAUSearch != null && $cachedAUSearch['plugin_status'] == 'active') ? 'selected' : ''; ?>><?php esc_html_e( 'Active', 'mainwp' ); ?></option>
                  <option value="inactive" <?php echo ( $cachedAUSearch != null && $cachedAUSearch['plugin_status'] == 'inactive') ? 'selected' : ''; ?>><?php esc_html_e( 'Inactive', 'mainwp' ); ?></option>
                </select>
              </div>
            </div>
  				</div>
          ...
          <div class="mainwp-search-options">
              ...
              <div class="field">
                <select class="ui fluid dropdown" id="mainwp_au_plugin_trust_status">
        					<option value="all" <?php echo ( $cachedAUSearch != null && $cachedAUSearch['status'] == 'all') ? 'selected' : ''; ?>><?php esc_html_e( 'Trusted, Not trusted and Ignored', 'mainwp' ); ?></option>
        					<option value="trust" <?php echo ( $cachedAUSearch != null && $cachedAUSearch['status'] == 'trust') ? 'selected' : ''; ?>><?php esc_html_e( 'Trusted', 'mainwp' ); ?></option>
                  <option value="untrust" <?php echo ( $cachedAUSearch != null && $cachedAUSearch['status'] == 'untrust') ? 'selected' : ''; ?>><?php esc_html_e( 'Not trusted', 'mainwp' ); ?></option>
                  <option value="ignored" <?php echo ( $cachedAUSearch != null && $cachedAUSearch['status'] == 'ignored') ? 'selected' : ''; ?>><?php esc_html_e( 'Ignored', 'mainwp' ); ?></option>
        				</select>
        			</div>
        			<div class="field">
        				<div class="ui input fluid">
      					<input type="text" placeholder="<?php esc_attr_e( 'Containing keyword', 'mainwp' ); ?>" id="mainwp_au_plugin_keyword" class="text" value="<?php echo ( $cachedAUSearch !== null ) ? $cachedAUSearch['keyword'] : ''; ?>">
        				</div>
        			</div>
        		</div>
  				</div>
    ...
    <table class="ui single line table" id="mainwp-all-active-plugins-table">
      ...
      <?php foreach ( $plugins as $slug => $plugin ) : ?>
        ...
        <tr plugin-slug="<?php echo urlencode( $slug ); ?>" plugin-name="<?php echo strip_tags( $name ); ?>">
          ...
          <td><?php echo ( $plugin['active'] == 1 ) ? __( 'Active', 'mainwp' ) : __( 'Inactive', 'mainwp' ); ?></td>
          <td><?php echo ( in_array( $slug, $trustedPlugins ) ) ? '<span class="ui mini green fluid center aligned label">' . __( 'Trusted', 'mainwp' ) . '</span>' : '<span class="ui mini red fluid center aligned label">' . __( 'Not Trusted', 'mainwp' ) . '</span>'; ?></td>
          <td><?php echo ( isset( $decodedIgnoredPlugins[$slug] ) ) ? '<span class="ui mini label">' . __( 'Ignored', 'mainwp' ) . '</span>' : '' ; ?></td>
      ...
      <table id="mainwp-globally-ignored-plugins" class="ui compact selectable table stackable">
        ...
        <?php foreach ( $decodedIgnoredPlugins as $ignoredPlugin => $ignoredPluginName ) : ?>
          <tr plugin-slug="<?php echo urlencode( $ignoredPlugin ); ?>">
            <td><a href="<?php echo admin_url() . 'plugin-install.php?tab=plugin-information&plugin=' . urlencode( dirname( $ignoredPlugin ) ) . '&TB_iframe=true&width=640&height=477'; ?>" target="_blank"><?php esc_html_e( $ignoredPluginName ); ?></a></td>
            <td><?php esc_html_e( $ignoredPlugin ); ?></td>
        ...
        <?php if ( $cnt > 0 ) : ?>
          ...
          $first = true;
            ...
            <tr site-id="<?php echo $website->id; ?>" plugin-slug="<?php echo urlencode( $ignoredPlugin ); ?>">
              ...
              <td><?php esc_html_e( $ignoredPlugin ); ?></td>
              <?php if ( mainwp_current_user_can( 'dashboard', 'ignore_unignore_updates' ) ) : ?>
                <td class="right aligned"><a href="#" class="ui mini button" onClick="return updatesoverview_plugins_unignore_detail('<?php echo urlencode( $ignoredPlugin ); ?>', <?php esc_attr_e( $website->id ); ?>)"> <?php esc_html_e( 'Unignore', 'mainwp' ); ?></a></td>
              <?php endif; ?>
                                                    </tr>
                                <?php
                            }
      ...
      <table id="mainwp-per-site-ignored-abandoned-plugins" class="ui compact selectable table stackable">
        ...
        <?php if ( $cnt > 0 ) : ?>
          ...
          while ( $websites && ( $website = @MainWP_DB::fetch_object( $websites ) ) ) {
            ...
            foreach ( $decodedIgnoredPlugins as $ignoredPlugin => $ignoredPluginName ) {
                                ?>
              <tr site-id="<?php esc_attr_e( $website->id ); ?>" plugin-slug="<?php echo urlencode( $ignoredPlugin ); ?>">
                <?php if ( $first ) : ?>
                <td><div><a href="<?php echo admin_url( 'admin.php?page=managesites&dashboard=' . $website->id ); ?>"><?php echo stripslashes( $website->name ); ?></a></div></td>
                <?php $first = false; ?>
                <?php else : ?>
                ...
                <td><a href="<?php echo admin_url() . 'plugin-install.php?tab=plugin-information&plugin=' . urlencode( dirname( $ignoredPlugin ) ) . '&TB_iframe=true&width=640&height=477'; ?>" target="_blank"><?php esc_html_e( $ignoredPluginName ); ?></a></td>
                <td><?php esc_html_e( $ignoredPlugin ); ?></td>
                <td class="right aligned"><a href="#" class="ui mini button" onClick="return updatesoverview_plugins_unignore_abandoned_detail('<?php echo urlencode( $ignoredPlugin ); ?>', <?php esc_attr_e( $website->id ); ?>)"> <?php esc_html_e( 'Unignore', 'mainwp' ); ?></a></td>
                                                    </tr>
                                <?php
                            }
                        }
  ...
  public static function trustPlugin( $slug ) {
    ...
    // Hook the section help content to the Help Sidebar element
  	public static function mainwp_help_content() {
  		if ( isset( $_GET['page'] ) && ( $_GET['page'] == "PluginsManage" || $_GET['page'] == "PluginsInstall" || $_GET['page'] == "PluginsAutoUpdate" || $_GET['page'] == "PluginsIgnore" || $_GET['page'] == "PluginsIgnoredAbandoned" ) ) {
  			?>
  			<p><?php echo __( 'If you need help with managing plugins, please review following help documents', 'mainwp' ); ?></p>
