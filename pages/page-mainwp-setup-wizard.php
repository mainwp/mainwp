...
class MainWP_Setup_Wizard {
	...
	public function setup_wizard_steps() {
		...
		<div id="mainwp-quick-setup-wizard-steps" class="ui tablet stackable ordered vertical steps" style="float: left; margin-right: 3em;">
			<?php foreach ( $ouput_steps as $step_key => $step ) : ?>
				<?php if ( isset( $step[ 'hidden' ] ) && $step[ 'hidden' ] )
					continue; ?>
				<div class="step <?php if ( $step_key === $this->step ) { echo 'active'; } elseif ( array_search( $this->step, array_keys( $this->steps ) ) > array_search( $step_key, array_keys( $this->steps ) ) ) { echo 'completed'; } ?>">
					<div class="content">
	...
	public function setup_wizard_content() {
		echo '<div class="mainwp-quick-setup-wizard-steps-content" style="float:right; width:77%;">';
		...
		<form method="post" class="ui form">
			...
			<div id="mainwp-quick-setup-opessl-location" class="grouped fields" <?php echo $systemType == 3 ? '' : 'style="display:none"'; ?>>
				...
				<div class="field" id="mainwp-setup-installation-openssl-location">
					...
					<div><em><?php echo sprintf( __( '%sClick here%s to see how to find the OpenSSL.cnf file.', 'mainwp' ), '<a href="https://mainwp.com/help/docs/how-to-find-the-openssl-cnf-file/" target="_blank">', '</a>' ); ?></em></div>
				</div>
			</div>
			<div class="ui hidden divider"></div>
			<div class="ui hidden divider"></div>
		...
		<form method="post" class="ui form">
			...
			<div class="field">
				...
				<select name="mwp_setup_type_hosting" id="mwp_setup_type_hosting" class="ui dropdown">
					<option value="3" <?php if ( false == $typeHosting || 3 == $typeHosting ) { ?>selected<?php } ?>><?php _e( 'Shared hosting environment', 'mainwp' ); ?></option>
					<option value="1" <?php if ( $typeHosting == 1 ) { ?>selected<?php } ?>><?php _e( 'Virtual Private Server (VPS)', 'mainwp' ); ?></option>
					<option value="2" <?php if ( $typeHosting == 2 ) { ?>selected<?php } ?>><?php _e( 'Dedicated server', 'mainwp' ); ?></option>
				</select>
			</div>
			<?php endif; ?>
			<div class="field">
				...
				<select name="mwp_setup_manage_planning" id="mwp_setup_manage_planning" class="ui dropdown">
					<option value="1" <?php if ( ( $managePlanning == false ) || ( $managePlanning == 1 ) ) {?>selected<?php } ?>><?php _e( 'Less than 50 websites', 'mainwp' ); ?></option>
					<option value="2" <?php if ( $managePlanning == 2 ) { ?>selected<?php } ?>><?php _e( 'More than 50 websites', 'mainwp' ); ?></option>
				</select>
			</div>
			<div class="ui hidden divider"></div>
			<div class="ui hidden divider"></div>
		...
		<form method="post" class="ui form">
			...
			<div class="field" id="mwp_setup_tr_backup_method" <?php echo $style; ?>>
				...
				<select class="ui dropdown" name="mwp_setup_backup_method" id="mwp_setup_backup_method">
					<option></option>
					<option value="updraftplus" <?php if ( $backup_method == 'updraftplus' ): ?>selected<?php endif; ?>>UpdraftPlus (<?php _e( 'Free Extension', 'mainwp' ); ?>)</option>
					<option value="backupwp" <?php if ( $backup_method == 'backupwp' ): ?>selected<?php endif; ?>>BackUpWordPress (<?php _e( 'Free Extension', 'mainwp' ); ?>)</option>
					<option value="backwpup" <?php if ( $backup_method == 'backwpup' ): ?>selected<?php endif; ?>>BackWPup (<?php _e( 'Free Extension', 'mainwp' ); ?>)</option>
							</select>
			</div>
			<div class="ui hidden divider"></div>
	...
	public function uptime_robot_add_monitor( $apiKey, $contact_id, $monitor_url ) {
		...
		if ( is_object( $result ) ) {
			if ( property_exists( $result, 'stat' ) && $result->stat == 'ok' ) {
				return true;
			} else if ( property_exists( $result, 'error' ) &&  property_exists( $result->error, 'message' )) {
