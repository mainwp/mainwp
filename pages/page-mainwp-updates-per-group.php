				...
				<tr>
					...
					<th class="no-sort">
						...
						if ( MainWP_Updates::user_can_update_plugins() ) {
							...
							<a href="javascript:void(0)" onClick="return updatesoverview_plugins_global_upgrade_all();" class="mainwp-update-all-button ui mini green button" data-tooltip="<?php esc_html_e( 'Update all sites.', 'mainwp' ); ?>" data-inverted="" data-position="top right"><?php esc_html_e( 'Update All Plugins' ); ?></a>
								<?php
							}
						}
						?>
											...
											<td colspan="5">
												<table id="mainwp-wordpress-updates-plugins-inner-table" class="ui table mainwp-manage-updates-table mainwp-updates-list">
													...
													<?php foreach ( $plugin_upgrades as $slug => $plugin_upgrade ) : ?>
														...
														?>
														<tr class="mainwp-plugin-update" plugin_slug="<?php echo esc_attr( $plugin_name); ?>" premium="<?php echo ( isset( $plugin_upgrade['premium'] ) ? $plugin_upgrade['premium'] : 0 ) ? 1 : 0; ?>" updated="0">
															<?php
												...
												<table id="mainwp-wordpress-updates-themes-inner-table" class="ui table mainwp-manage-updates-table mainwp-updates-list">
													...
													<?php foreach ( $theme_upgrades as $slug => $theme_upgrade ) : ?>
														...
														<?php
															...
															?>
														<tr class="mainwp-theme-update" theme_slug="<?php echo esc_attr( $theme_name ); ?>" premium="<?php echo ( isset( $theme_upgrade['premium'] ) ? $theme_upgrade['premium'] : 0 ) ? 1 : 0; ?>" updated="0">
															<?php
															$row_columns     = $updates_table_helper->render_columns( $row_columns, $website );
															$action_rendered = isset( $row_columns['action'] ) ? true : false;
															if ( ! $action_rendered ) :
															...
															<?php endif; ?>
																<?php if ( MainWP_Updates::user_can_update_themes() ) : ?>
																<a href="javascript:void(0)" class="mainwp-update-now-button ui mini green button" onClick="return updatesoverview_themes_upgrade( '<?php echo esc_js( $theme_name )eme_name )eme_name )eme_name )eme_name )eme_name )eme_name )eme_name ); ?>', <?php echo intval( $website->id ); ?> )"><?php esc_html_e( 'Update Now', 'mainwp' ); ?></a>
															<?php endif; ?>
															</td>
															<?php endif; ?>
														</tr>
											...
											<tbody site_id="<?php echo esc_attr( $website->id ); ?>" site_name="<?php echo rawurlencode( stripslashes( $website->name ) ); ?>">
												<?php foreach ( $plugins_outdate as $slug => $plugin_outdate ) : ?>
													...
													<tr dismissed="0">
														...
														<td>
															<a href="<?php echo admin_url() . 'plugin-install.php?tab=plugin-information&wpplugin=' . intval( $website->id ) . '&plugin=' . dirname( $slug ) . '&url=' . ( isset( $plugin_outdate['PluginURI'] ) ? esc_html( rawurlencode( $plugin_outdate['PluginURI'] ) ) : '' ) . '&name=' . esc_html( rawurlencode( $plugin_outdate['Name'] ) ); ?>" target="_blank" class="open-plugin-details-modal"><?php echo esc_html( $plugin_outdate['Name'] ); ?></a>
