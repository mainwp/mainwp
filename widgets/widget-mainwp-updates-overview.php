...
//           $pluginsIgnored_perSites = $themesIgnored_perSites = array();
			...
			if ( is_array( $wp_upgrades ) && count( $wp_upgrades ) > 0 ) {
				$total_wp_upgrades ++;
                $all_wp_updates[] = array('id' => $website->id, 'name' => $website->name);
			}
			...
			if ( is_array( $translation_upgrades ) ) {
                ...
                if ( count( $translation_upgrades ) > 0 ) {
                    foreach ( $translation_upgrades as $trans_upgrade ) {
                        ...
                        $all_translations_updates[] = array( 'id' => $website->id, 'name' => $website->name, 'translation_slug' => $slug );
                    }
                }
			...
			if ( is_array( $plugin_upgrades ) ) {
                ...
                if ( count( $plugin_upgrades ) > 0  ) {
                    foreach ( $plugin_upgrades as $slug => $value ) {
                        $all_plugins_updates[] = array( 'id' => $website->id, 'name' => $website->name, 'plugin_slug' => $slug );
                    }
                }
			}
			...
			if ( is_array( $theme_upgrades ) ) {
                ...
                if ( count( $theme_upgrades ) > 0 ) {
                    foreach ( $theme_upgrades as $slug => $value ) {
                        $all_themes_updates[] = array( 'id' => $website->id, 'name' => $website->name, 'theme_slug' => $slug );
                    }
                }
			}
		...
		<input type="hidden" name="updatesoverview_limit_updates_all" id="updatesoverview_limit_updates_all" value="<?php echo intval( $limit_updates_all ); ?>">
				...
				<?php if ( $user_can_update_wordpress && $user_can_update_plugins && $user_can_update_themes && $user_can_update_translation ) : ?>
					<?php if (! get_option( 'mainwp_hide_update_everything', false ) ) : ?>
                            <a href="#" <?php if ( $total_upgrades == 0 ) { echo 'disabled'; } else { ?> onClick="return updatesoverview_global_upgrade_all( 'all' );"  <?php } ?> class="ui big button fluid green" data-tooltip="<?php _e( 'Clicking this button will update all Plugins, Themes, WP Core files and translations on All your websites.', 'mainwp' ); ?>" data-inverted="" data-position="top center"><?php _e( 'Update Everything', 'mainwp' ); ?></a>
					<?php endif; ?>
				<?php endif; ?>
						</div>
